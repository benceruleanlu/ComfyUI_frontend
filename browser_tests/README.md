...
This `.env` file will not exist until you create it yourself.

A template with helpful information can be found at `.env_example`.

### Parallel Tests
By default, Playwright Test runs tests in parallel with several worker processes.

If you are running Playwright tests in parallel, the flag `--multi-user` must be added to the main ComfyUI process.
